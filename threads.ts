...
const example = async () => {
    ...
    const db = await Database.withKeyInfo(keyInfo, threadID.toString())
    ...
    const chat = await db.collections.get(roomName) || await db.newCollection<Message>(roomName, messageSchema);
